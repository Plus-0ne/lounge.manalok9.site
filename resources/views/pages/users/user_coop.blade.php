...
<body>
                            ...
                            <div class="p-4 rounded-3 bg-light section_shadow border border-1 border-light">
                                <div class="row">
                                    ...
                                    <div class="col-6 col-sm-4">
                                        <input type="text" name="petno" class="form-control" placeholder="Bird IAGD #" required>
                                    </div>
                                    <div class="col-6 col-sm-5">
                                        <button type="submit" class="btn btn-primary"><i class="mdi mdi-plus"></i> Add Bird</button>
                                    </div>
                                </div>
                            </div>
                        </form>
                        ...
                        <div class="py-4 px-0 rounded-3 bg-light section_shadow border border-1 border-light">
                            <div class="row">
                                <div class="col-12 text-start px-5 ms-sm-1 ms-md-3 ms-lg-5 row" style="margin-top: -3rem;">
                                        ...
                                        <h5>
                                            <div class="dropdown">
                                                <button class="dropdown-toggle text-light py-3 px-4" style="background-color: #1e2530;" type="button" data-bs-toggle="dropdown">
                                                    <i class="mdi mdi-bird pe-1"></i> Birds
                                                </button>
                                    ...
                                    <div class="col-auto mx-auto mx-sm-0">
                                        <a href="coop_unregistered" class="btn btn-success rounded-pill p-3 py-2 py-sm-3 mb-1 fw-bold">
                                            <i class="mdi mdi-bird"></i> PRE-REGISTERED BIRDS ({{ $data['coop_count'] }})
                                        </a>
                                    </div>
                                    <div class="col-auto mx-auto mx-sm-0">
                                        <button type="button" class="btn btn-primary btn-size-95-rem shadow p-3 py-2 py-sm-3 mb-1"
                                            data-bs-target="#add_bird_unregistered" data-bs-toggle="modal">
                                            <i class="mdi mdi-plus-box-outline"></i> REGISTER BIRD
                                        </button>
                                    </div>
                                </div>
                            </div>
                            <div class="col-12 col-sm-12 col-md-12 px-0 pt-4 pb-4">
                                <div class="table-responsive">
                                    <table id="member_birds" class="table table-striped w-100">
                                        <thead>
                                            <tr>
                                        ...
                                        <tbody>
                                            @if(!empty($data['member_birds']) && count($data['member_birds']) > 0)
                                                @foreach ($data['member_birds'] as $row)
                                                    <tr>
                                                        ...
                                                        <td class="text-center align-middle pe-5">
                                                            ...
                                                            @else
                                                                <a href="{{ empty($row->PetNo) ? '#' : 'coop_info/' . $row->PetNo }}" class="btn btn-primary {{ (empty($row->PetNo) ? 'disabled' : '') }}">
                                                                    Info
                                                                </a>
                                                            @endif
                                                            <div class="mb-3">
    ...
    @include('pages/users/template/modals/modal-add-bird-unregistered')
</body>
    ...
    $(document).ready(function() {
        @if (Session::has('response'))
            @switch(Session::get('response'))
                ...
                @case('bird_added')
                    toastr["success"]("Bird added", "Success");
                @break
                @case('error_bird_add')
                    toastr["error"]("Something's wrong! Please try again later.", "Error");
                @break
                @case('bird_approval_pending')
                    toastr["warning"]("Bird approval pending", "Warning");
                @break
                @case('bird_claimed')
                    toastr["warning"]("Bird has already been claimed", "Warning");
                @break
                @case('bird_not_found')
                    toastr["warning"]("Bird not found", "Warning");
                @break

                @case('pet_added')
                    toastr["success"]("Bird registration added", "Success");
                @break
                @case('error_pet_add')
                    toastr["error"]("Something's wrong! Please try again later.", "Error");
                @break
