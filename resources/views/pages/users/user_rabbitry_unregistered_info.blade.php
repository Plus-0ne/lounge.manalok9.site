...
<body>
                        ...
                        <div class="p-4 gallery_container d-flex flex-wrap">
                            <div class="row px-5">
                                <h5 class="text-light py-3 px-4" style="background-color: #1e2530; margin-top: -2.8rem;">
                                    <a href="{{ url('rabbitry_unregistered') }}" class="me-2">
                            ...
                            <div class="col-12 my-2 d-flex flex-row justify-content-end">
                                @if ($data['pet_data']->Status == 2)
                                <div class="me-2">
                                    <a class="btn btn-primary btn-sm" href="{{ route('user.animal_certifcation') }}?PetUUID={{ $data['pet_data']->PetUUID }}">
                                        <span class="mdi mdi-certificate"></span> Request certificate
                                    </a>
                                </div>
                                @endif
                                    <div class="dropdown d-inline">
                                        <button class="btn btn-info fw-bold btn-sm" type="button" data-bs-toggle="dropdown">
                            ...
                            <div class="col-12 my-2 text-end">
                                @if ($data['pet_data']->OwnerUUID == Auth::guard('web')->user()->uuid || (!empty($data['pet_data']->OwnerIAGDNo) && $data['pet_data']->OwnerIAGDNo == Auth::guard('web')->user()->iagd_number))
                                    @if ($data['pet_data']->Status == 4)
                                        <button type="button" class="btn btn-success btn-size-95-rem shadow"
                                            data-bs-target="#update_rabbit_unregistered" data-bs-toggle="modal">
                                            <i class="mdi mdi-checkbox-marked-outline"></i> VERIFY REGISTRATION
                                        </button>
                                    @elseif ($data['pet_data']->Status == 1)
                                        <div class="row ">
                                            <div class="mx-auto p-3 col-auto bg-secondary text-light rounded-1">
                            ...
                            <div class="row p-3 w-100">
                                {{-- <div class="col-12 col-sm-12 col-xxl-4">
                                    <div class="row px-4">
                                        <img id="pet-img" class="shadow p-0 mx-auto" src="{{ asset($data['pet_image'] ?? 'img/no_img.jpg') }}" style="max-width: 350px;">
                                    </div>
                                    <div class="row px-5 py-3">
                                        <button class="btn btn-primary btn-size-95-rem btn-pet-image"
                                            data-pet_no="{{ $data['pet_data']->PetUUID }}" data-pet_type="unregistered_rabbit"
                                            data-bs-target="#upload_img_pet" data-bs-toggle="modal">
                                            <i class="mdi mdi-square-edit-outline"></i> UPDATE IMAGE
                                        </button>
                                    </div>
                                </div> --}}
                                <div class="col-12 col-sm-12">
                                    <div class="row">
                                        <input id="file_token" type="hidden" value="{{ $data['adtl_data']->FileToken ?? '- - - - -' }}">
                                    ...
                                    @if ($data['pet_data']->OwnerUUID == Auth::guard('web')->user()->uuid || (!empty($data['pet_data']->OwnerIAGDNo) && $data['pet_data']->OwnerIAGDNo == Auth::guard('web')->user()->iagd_number))
                                        ...
                                        <div class="row adtl_info d-none">
                                            ...
                                            <dl class="col-12 col-sm-12 col-md-12 col-lg-12">
                                                <dt class="text-center text-md-start">
                                                    <i class="mdi mdi-file-upload mdi-24px pe-1"></i> Rabbit's veterinary record:
                                                </dt>
                                                <dd class="pe-md-3 pe-xl-5">
                                                    <div class="text-center text-md-start shadow px-4 py-3">
    ...
    @if ($data['pet_data']->OwnerUUID == Auth::guard('web')->user()->uuid || (!empty($data['pet_data']->OwnerIAGDNo) && $data['pet_data']->OwnerIAGDNo == Auth::guard('web')->user()->iagd_number))
        @include('pages/users/template/modals/modal-update-rabbit-unregistered')
    @endif
    ...
    $(document).ready(function() {
        ...
        @if (Session::has('response'))
            @switch(Session::get('response'))
                ...
                @break
                @case('rabbit_fail_update')
                    toastr["warning"]('Updating rabbit info failed!');
                @break
                @case('pet_not_found')
                    toastr["warning"]('Pet not found');
                @break
                @case('rabbit_update_not_allowed')
                    toastr["warning"]('Updating info not allowed.');
                @break
